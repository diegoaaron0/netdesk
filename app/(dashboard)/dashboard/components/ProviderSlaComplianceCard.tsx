  ...
  return (
    <div style={{ background: 'white', border: '0.5px solid #e5e7eb', borderRadius: '12px', padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        <span style={{ fontSize: '12px', fontWeight: 600, color: '#0f172a', marginRight: 'auto' }}>E. Cumplimiento SLA</span>
        {!loading && g != null && (
      ...
      {error && (
        <div style={{ fontSize: '12px', color: '#A32D2D', padding: '8px 0', textAlign: 'center' }}>Error al cargar datos</div>
      )}
            ...
            <thead>
              <tr style={{ borderBottom: '0.5px solid #e5e7eb' }}>
                {[
                  ...
                  { label: 'Fuera',     align: 'center' as const },
              ...
              {proveedores.slice(0, 6).map((p, i) => {
                ...
                return (
                  <tr key={p.proveedorId} style={{ borderTop: i > 0 ? '0.5px solid #f3f4f6' : 'none' }}>
                    <td style={{ padding: '4px 6px', fontWeight: 500, color: '#0f172a', whiteSpace: 'nowrap', maxWidth: '110px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.nombre}</td>

                    {/* SLA% barra + número */}
                    <td style={{ padding: '4px 6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '3px', justifyContent: 'center' }}>
                        <div style={{ width: '28px', height: '4px', background: '#e5e7eb', borderRadius: '3px', overflow: 'hidden' }}>
                    ...
                    <td style={{ padding: '4px 6px', textAlign: 'center', color: '#64748b' }}>{p.evaluables}</td>

                    <td style={{ padding: '4px 6px', textAlign: 'center', fontWeight: p.fueraSLA > 0 ? 600 : 400, color: p.fueraSLA > 0 ? '#A32D2D' : '#64748b' }}>
                      {p.fueraSLA > 0 ? p.fueraSLA : '—'}
                    </td>

                    <td style={{ padding: '4px 6px', textAlign: 'right', fontWeight: 500, color: respLenta ? '#BA7517' : '#0f172a', whiteSpace: 'nowrap' }}>
                      {fmtMin(p.tPromRespuestaMin)}{respLenta && <span style={{ fontSize: '8px', marginLeft: '2px' }}>↑</span>}
                    </td>
