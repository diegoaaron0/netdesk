...
export default function ReportesPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/usuarios').then(r => r.json()).then(() => {})
  }, [filtros])

  useEffect(() => {
    fetch('/api/tiendas?q=').then(() => {})
  }, [])
  ...
  function inputStyle() { return { padding: '6px 10px', fontSize: '12px', border: '0.5px solid var(--border)', borderRadius: '8px', background: 'var(--card)', color: 'var(--foreground)', outline: 'none' } }
