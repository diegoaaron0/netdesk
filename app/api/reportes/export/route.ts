...
export async function GET(req: Request) {
  ...
  const rows = await db.execute(sql`
    SELECT
      ...
      COALESCE(i.tipo_operacion_manual::text, i.factor_operativo::text)                   AS factor_operativo,
      ...
      i.comentarios,
      COALESCE(i.cont_rendimiento::text, i.mov_rendimiento::text)                         AS efectividad_contingencia,
      i.mttr_minutos                                                                       AS mttr_min,
      CASE
        WHEN n1.hora_correo_raw IS NULL
          THEN 'No escalado'
        WHEN EXTRACT(EPOCH FROM (n1.hora_correo_raw - i.hora_registro)) / 60 <= 60
          THEN 'Cumplido'
        ELSE 'Incumplido'
      ...
      CASE
        WHEN i.evaluable_proveedor = false
          THEN 'No evaluable'
        WHEN n1.hora_correo_raw IS NOT NULL
          AND EXTRACT(EPOCH FROM (n1.hora_correo_raw - i.hora_registro)) / 60 <= 60
          ...
          THEN 'Sí'
        WHEN (
          n1.hora_correo_raw IS NOT NULL
          AND EXTRACT(EPOCH FROM (n1.hora_correo_raw - i.hora_registro)) / 60 > 60
        ) OR (
          i.estado = 'RESUELTO'
          AND i.mttr_minutos IS NOT NULL
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        MIN(e.hora_envio_correo)                                                              AS hora_correo_raw
      FROM escalamientos e
