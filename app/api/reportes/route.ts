  ...
  const [tot, totAnt, byDay, byTipo, mttrProv, slaProv, topTiendas, reinc, byZona, slaTend, mttrZonaMes] = await Promise.all([
    db.execute(sql`
      SELECT COUNT(*)::int AS total,
        ROUND(AVG(mttr_minutos))::int AS mttr_avg,
        ROUND(COUNT(*) FILTER (WHERE mttr_minutos <= 240 AND estado = 'RESUELTO') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE estado = 'RESUELTO'), 0))::int AS sla_pct,
        COUNT(DISTINCT tienda_id)::int AS tiendas
      FROM incidentes
    ...
    `),
    db.execute(sql`
      SELECT p.nombre, COUNT(i.id)::int AS total, ROUND(AVG(i.mttr_minutos))::int AS mttr_avg
      FROM incidentes i
      ...
      GROUP BY p.id, p.nombre ORDER BY mttr_avg ASC
    `),
    db.execute(sql`
      SELECT p.nombre, COUNT(i.id)::int AS total,
        ROUND(COUNT(*) FILTER (WHERE i.mttr_minutos <= 240 AND i.estado = 'RESUELTO') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE i.estado = 'RESUELTO'), 0))::int AS sla_pct
      FROM incidentes i
      JOIN tiendas t ON i.tienda_id = t.id
      JOIN proveedores p ON t.proveedor_id = p.id
      WHERE i.hora_registro >= ${desde}::timestamptz AND i.hora_registro < ${hasta}::timestamptz
      GROUP BY p.id, p.nombre HAVING COUNT(i.id) > 0
      ORDER BY sla_pct DESC NULLS LAST
    `),
    db.execute(sql`
      SELECT t.codigo, t.nombre_cc AS nombre, t.distrito, p.nombre AS proveedor,
        COUNT(i.id)::int AS total, ROUND(AVG(i.mttr_minutos))::int AS mttr_avg
      FROM incidentes i
      JOIN tiendas t ON i.tienda_id = t.id
      JOIN proveedores p ON t.proveedor_id = p.id
      WHERE i.hora_registro >= ${desde}::timestamptz AND i.hora_registro < ${hasta}::timestamptz
      GROUP BY t.id, t.codigo, t.nombre_cc, t.distrito, p.nombre
