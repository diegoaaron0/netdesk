...
export function CronometroPrincipal({ horaRegistro, horaFin, tiempoAcumuladoMin, horaRegistroOriginal, compacto = false }: { horaRegistro: Date | string; horaFin: Date | string | null; tiempoAcumuladoMin?: number | null; horaRegistroOriginal?: Date | string | null; compacto?: boolean }) {
  ...
  if (compacto) {
    return (
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', whiteSpace: 'nowrap' }}
        ...
        <span style={{ fontSize: '9px', color: 'var(--faint-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
          {etiqueta}
        </span>
        <span style={{ fontSize: '19px', fontWeight: 500, fontFamily: 'monospace', color: detenido ? 'var(--ok)' : 'white', letterSpacing: '0.03em', lineHeight: 1 }}>
          {display}
        </span>
      </div>
    )
  ...
  return (
    <div style={{ textAlign: 'right' }}>
      ...
      <div style={{ fontSize: '32px', fontWeight: 500, fontFamily: 'monospace', color: detenido ? 'var(--ok)' : 'white', letterSpacing: '0.05em', lineHeight: 1 }}>
        {display}
      </div>
